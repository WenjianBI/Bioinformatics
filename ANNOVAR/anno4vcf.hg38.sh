...
## NOTE: WE UNIFORMLY USE HG38. FOR LIFTOVER, WE SUGGEST "CROSSMAP" SOFTWARE

if (($#==0))
then
echo "/home/wbi1/Pipeline_BWJ/ANNOTATION.sh DIR VCF PREFIX"
exit
fi
...
VCF=$2    # input .vcf/.vcf.gz files, should include directory
...
table_annovar.pl ${PREFIX}.avinput ${DIR_DB} -out ${PREFIX}.anno -build hg38 -remove -protocol refGene,cytoBand,avsnp147,gwasCatalog -operation g,r,f,r -nastring . -otherinfo

if (($#==4))
then
table_annovar.pl ${PREFIX}.avinput ${DIR_DB} -out ${PREFIX}.bed -build hg38 -remove ${BED} 
